...
def get_number(debug: bool, file_path: str) -> str:
    ...
    try:
        ...
        #     return file_number
            ...
            # 欧美番号匹配规则
            oumei = re.search(r'[a-zA-Z]+\d{0,2}\.\d{2}\.\d{2}\.\d{2}', filepath)
            if oumei:
                return oumei.group()
            try:
                return str(
...
G_TAKE_NUM_RULES = {
    ...
    r'(?<!se)xart': lambda x: str(re.search(r'xart\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('xart', 'x-art')),
    ...
    r'dpg\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'dpg\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('dpg', 'digitalPlayground')),
    r'tlib\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'tlib\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('tlib', 'TeensLikeItBig')),
    r'btaw\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'btaw\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('btaw', 'BigTitsatWork')),
    r'bgb\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'bgb\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('bgb', 'BabyGotBoobs')),
    r'bex\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'bex\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('bex', 'BrazzersExxtra')),
    r'tlhc\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'tlhc\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('tlhc', 'TeensLoveHugeCocks')),
    r'sgs\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'sgs\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('sgs', 'ShesGonnaSquirt')),
    # r'cs\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'cs\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('cs', 'CaptainStabbin')),
    r'moc\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'moc\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('moc', 'Monstersofcock')),
    r'ddm\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'ddm\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('ddm', 'DareDorm')),
    r'inh\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'inh\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('inh', 'InnocentHigh')),
    r'zzs\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'zzs\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('zzs', 'ZZSeries')),
    r'mbt\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'mbt\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('mbt', 'MomsBangTeens')),
    r'rws\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'rws\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('rws', 'RealWifeStories')),
    r'ham\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'ham\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('ham', 'HotAndMean')),
    r'(?<!so)da\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'da\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('da', 'DoctorAdventures')),
    r'wlt\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'wlt\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('wlt', 'WeLiveTogether')),
    r'tgs\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'tgs\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('tgs', 'ThisGirlSucks')),
    r'mbc\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'mbc\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('mbc', 'MyBabySittersClub')),
    r'ap\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'ap\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('ap', 'AssParade')),
    r'pubb\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'pubb\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('pubb', 'PublicBang')),
    r'btas\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'btas\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('btas', 'BigTitsatSchool')),
    r'plib\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'plib\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('plib', 'PornstarsLikeitBig')),
    r'dwp\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'dwp\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('dwp', 'DayWithAPornstar')),
    r'mlib\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'mlib\.\d{2}\.\d{2}\.\d{2}', x, re.I).group().replace('mlib', 'MilfsLikeItBig')),
    r'she\'snew\.\d{2}\.\d{2}\.\d{2}': lambda x: str(re.search(r'she\'snew\.\d{2}\.\d{2}\.\d{2}', x, re.I).group()),
...
if __name__ == "__main__":
    ...
    # print(get_number(True, "/home-soft/mnt/wd16/s/jav/rkprime.22.06.27.simon.kitty.and.mary.bambola.titillating-MP4-XXX.tourists.mp4"))
    # print(get_number(True, "/home-soft/mnt/wd16/s/jav/bangbros18.21.06.27.kylie.quinn.mp4"))
    print(get_number(True, "/home-soft/mnt/wd16/s/jav/seaxart.20.09.16.lulu.love.affair.of.the.heart.mp4"))
    # test_use_cases = (
    #     "MEYD-594-C.mp4",
    #     "SSIS-001_C.mp4",
    #     "SSIS100-C.mp4",
